...
func testTagMarks(
	...
	if markup.count < 3 {
		...
		if rightFlanking {
			...
			while i >= 0 {
				...
				if prevDelimiter.handled != true {
					if prevDelimiter.markup == byteString(char) && prevDelimiter.length == markup.count {
		...
		if leftFlanking {
			...
			state.i += markup.count
			state.delimiters.append(Delimiter(markup: byteString(char), start: start, length: markup.count, handled: nil, precedence: precedence))
