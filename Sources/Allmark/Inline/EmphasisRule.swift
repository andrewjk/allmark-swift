...
func testEmphasis(state: inout InlineParserState, parent: inout MarkdownNode) -> Bool {
	...
	if !state.isEscaped && (char == ASTERISK_CODE || char == UNDERSCORE_CODE) {
		let start = state.i
		var end = state.i
		...
		let codeBefore = start > 0 ? scalarValue(src, start - 1) : 0
		...
		let codeAfter = end + 1 < src.count ? scalarValue(src, end + 1) : 0
		...
		while i >= 0 {
			...
			if prevDelimiter.handled != true {
				if prevDelimiter.markup == byteString(char) {
		...
		if let startDel = startDelimiter {
			let canClose =
				...
				startDel.markup == byteString(char) &&
				// "Emphasis with _ is not allowed inside words"
				(char != UNDERSCORE_CODE || spaceAfter || punctuationAfter) &&
		...
		let canOpen =
			leftFlanking &&
			// "Emphasis with _ is not allowed inside words"
			(char != UNDERSCORE_CODE || spaceBefore || punctuationBefore)
		...
		if canOpen {
			...
			state.delimiters.append(Delimiter(markup: byteString(char), start: start, length: markup.count, handled: nil, precedence: emphasisRule.precedence))
