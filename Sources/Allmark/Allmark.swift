...
public enum Allmark {
	...
	public static func render(doc: MarkdownNode, renderers: [Renderer] = htmlRenderers, options: RenderOptions? = nil) -> String {
		return _render(doc: doc, renderers: renderers, options: options)
	}
	...
	/// - Returns: The rendered output string.
	public static func transform(src: String, rules: RuleSet, renderers: [Renderer], options: RenderOptions? = nil) -> String {
		return _transform(src: src, rules: rules, renderers: renderers, options: options)
	}
}
